...
const WordLevelUpload = () => {
  ...
  return (
    <div>
      ...
      <Table.Type27 data={wordLevelUploadData.data} setSelectedRowsData={setSelectedRowsData} />
    </div>
  )
}
...
const Header = ({ selectedRowsData }: { selectedRowsData: sentenceLevelUploadDT[] }) => {
  ...
  useEffect(() => {
    getAudioCheckerList("uploadAudioAnnotationWordChecker");
    getSpeakerList("uploadAudioAnnotationWordSpeaker");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
  ...
  return (
    <div className='ml-6 mr-4 mb-5 flex items-center justify-between'>
      <div>
        <h1 className='titleAudioManagement'>Word Annotation</h1>
