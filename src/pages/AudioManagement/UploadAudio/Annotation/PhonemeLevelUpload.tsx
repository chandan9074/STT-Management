...
const PhonemeLevelUpload = () => {
  ...
  return (
    <div>
      ...
      <Table.Type27 data={phonemeLevelUploadData.data} setSelectedRowsData={setSelectedRowsData} />
    </div>
  )
}
...
const Header = ({ selectedRowsData }: { selectedRowsData: sentenceLevelUploadDT[] }) => {
  ...
  useEffect(() => {
    getAudioCheckerList("uploadAudioAnnotationPhonemeChecker");
    getSpeakerList("uploadAudioAnnotationPhonemeSpeaker");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
  ...
  return (
    <div className='ml-6 mr-4 mb-5 flex items-center justify-between'>
      <div>
        <h1 className='titleAudioManagement'>Phoneme Annotation</h1>
        <p className='text-small text-ct-blue-90-70% mt-1.5'>List of audios that is needed to phoneme annotation</p>
      </div>
