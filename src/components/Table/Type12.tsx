...
import { recreateTableDT, ScriptItemDT } from "../../types/assignTypes";
...
const Type12 = () => {
  ...
  const { recreateTable, setRecreateTable, scriptForRecreate, setScriptForRecreate } = useAssigneeContext();
  const [openScriptModal, setOpenScriptModal] = useState<boolean>(false);


  const handleSelectItem = (item: ScriptItemDT) => {
    if (item?.id) {
        ...
        <div className="flex items-center gap-1 relative">
          ...
          {openScriptModal && data.script.id && (
            <div className="absolute bottom-11 right-0 w-[376px] bg-white rounded-md z-[100]">
              {data.script.id && <ScriptTargetModal handleSelectItem={handleSelectItem} selectedItemList={scriptForRecreate} selectedScriptId={data.script.id} selectedTargetId={""} />}
            </div>
          )}
        </div>
      ),
