...
import { assignAudioTrackDT, speechDt2 } from '../../types/assignTypes';
...
type Props = {
    ...
    setSpeechData: Dispatch<SetStateAction<speechDt2[]>>,
...
const AudioUpload = ({ data, audioId, speechData, setSpeechData, setAudioUploadStatus, isUpload }: Props) => {
    ...
    const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            const index = speechData.findIndex((item: speechDt2) => item?.id === audioId);
            if (index === -1) {
                return;
            }
