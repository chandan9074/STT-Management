...
const Graph = ({ data }: { data: createCollectSimilarPropertyDT[] }) => {
  return (
    <div className="flex flex-col h-full border-l border-blue-gray-20 py-2.5">
      {data.map((item, index) => (
        <div
          className={`flex items-start justify-start group relative ${
            data.length - 1 === index ? "" : "mb-[32px]"
          }`}
        >
          <div
          ...
          >
            <div
              className={`w-full h-full opacity-0 group-hover:opacity-100 rounded-r-full duration-300 group-hover:bg-gradient-to-r ${
                getTitleColor(item.name, false).split("bg-gradient-to-r")[1]
              }`}
            />
          </div>
